...
export interface IStorage {
  ...
  createQuizAttempt(attempt: InsertQuizAttempt): Promise<QuizAttempt>;
    ...
    getAdmins(): { username: string; isMainAdmin: boolean; addedAt: string; }[];
...
export class MemStorage implements IStorage {
  ...
  private admins: { username: string; isMainAdmin: boolean; addedAt: string; }[];
  ...
  // Admin management
  addAdmin(username: string): void {
    const exists = this.admins.some(admin => admin.username === username);
    if (!exists) {
      this.admins.push({
        username,
        isMainAdmin: false,
  ...
  isAdmin(username: string): boolean {
    return this.admins.some(admin => admin.username === username);
  }
